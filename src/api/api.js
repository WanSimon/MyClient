...
class API {
  ...
  open() {
    ...
    self.ws.onmessage = (msg) => {
      ...
      if (!msg || !msg.msgid || !msg.invokeid) {
        console.error("drop unknow message!", msg);
        return false;
      }
      ...
      if (msg.invokeid && message.hasOwnProperty(msg.msgid)) {
        ...
        if (req) {
          ...
          if (msg.errcode !== 0) {
            console.error("response err,err=", msg);
            req.reject(msg);
          } else {
  ...
  async init(){
    try {
      ...
      console.error('get_equipment_info failed,e=',e);
    }
  }
  ...
  watchScan() {
    ...
    document.onkeydown = async (e) => {
      if (this.mode !== 3) {
        console.warn('can not replenish product!');
        return;
      }
      //兼容性处理
      if (window.event) {
      ...
      if (nextCode == 13 && keycode != "" && nextTime - lastTime <= 30) {
        ...
        /*try {
          ...
          let res = await this.replenish_product(keycode);
          callback(res);
          //todo 高亮显示槽位
        } catch (e) {
          if (e.errcode == -400) {
            audioPlay(__static + '/该药品库存已满.mp3');
            console.log('药道库存已满！');
          }
          if (e.errcode == -401) {
  ...
  on_notify_equipment_status(body) {
    if (body.status != this.DeviceStatus.Normal) {
      console.error('设备状态异常,status=', body.status);
      router.push('/');
    }
  }

  on_notify_temperature_humidity(body) {
   ...
   * @param id_card 身份证号码
   * */
  async get_equipment_admin(id_card) {
    return this.request(message.get_equipment_admin, {id_card});
  }
    ...
    product_info_list:[
        {
            ...
            approval_number:"国药准号H11111"
        }
    ]
}
   * */
  async print_ticket(ticket_info) {
    return this.request(message.print_ticket, {ticket_info});
  }
