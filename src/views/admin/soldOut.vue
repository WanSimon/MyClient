    ...
    <div class="container" >
      <div class="search-input">
        ...
        <div v-show="keyVisible" id="keyboard" @click.stop="" class="dv-keyboard">
          <div v-for="key in keys" :key="key" class="key"  @click="inputKey(key)">{{key==0?"0":key}}
            <div class="del" v-if="key==0"></div>
          </div>
        </div>
        <div class="search-btn" @click="search()">查询</div>
      </div>
      <div class="table-head" style="margin-top: .8rem">
        <table>
          <colgroup>
            <col style="width: 12%" />
            <col style="width: 30%" />
...
<style scoped>
  ...
  .table-body{
    margin-top: .3rem;
    overflow-y: auto;
    max-height: 5.4rem;  /* TODO 高度根据实际情况自己确定 */
  }
