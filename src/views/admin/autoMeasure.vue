<template>
  <div class="measure"  @click="keyVisible =false" style="width: 100vw;height: 100vh;">
    ...
    <div class="filter-container">
      ...
      <div class="filter-item">
        <div class="search-input">
          ...
          <input class="search-txt" v-model="searchData.search_txt" readonly  @click.stop="keyVisible =!keyVisible" placeholder="请输入需要查找的药品名首字母" type="text"/>
    ...
    <div class="box02">
      ...
      <div style="width: 14%;float: left;">药道容量</div>
      <div style="width: 14%;float: left;">账面数量</div>
      <div style="width: 14%;float: left;">盘点数量</div>
      <div style="width: 18%;float: left;">实际数据</div>
    </div>
    <div class="box03">
      <div class="product-list">
        <div v-for="item in allList" class="box03-01" :key="`${item.x}-${item.y}`" v-show=" (searchData.row==='all' || searchData.row===(item.y+1))
          ...
          <div style="width: 25%;float: left;" class="box03-02">
            ...
            <div class="box03-01-02">
              <div style="font-weight: 600;color: #333333"><a href="javascript:;" @click.stop="detail(item)">{{item.name}}</a></div>
              <div>国际条码：{{item.bar_code}}</div>
              <div>规&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;格：{{item.specification}}</div>
            </div>
          </div>
          <div style="width: 14%;float: left;">{{item.upper_limit}}</div>
          <div style="width: 14%;float: left;">{{item.real_stock}}</div>
          <div style="width: 14%;float: left;">{{item.auto_measure_num}}</div>
          <div style="width: 18%;float: left;" class="box03-01-03">
            <div style="display:flex; width:100%;justify-content: space-around;align-items: center;">
              <div><el-input-number @focus="showKeyboard(item, $event)" v-model="item.measure_num" :min="0" :precision="0" :max="item.upper_limit" label="描述文字"></el-input-number></div>
    ...
    <div class="box04">
      ...
      <div class="box04-04" :class="{'btn-disabled': btnDisabled}" @click="beginMeasure">{{this.measuring?(btnDisabled ? '停止中...' : '停止盘点'):'开始盘点'}}</div>
    ...
    <digital-keyboard :origin-el-input="elInput" :visible.sync="keyboardVisible" @callback="callback" v-drag-dialog></digital-keyboard>
  </div>
</template>
  ...
  export default {
    ...
    async mounted(){
      ...
      this.api.on_notify_measure_channel = (body)=>{
        ...
        if(-1 === idx){
          console.warn('measrue slot can not find:', slot_no);
          return;
        }
    ...
    methods:{
      ...
      async selectSlot(item){
        try{
          ...
          console.error('选择相同批次号药道放对应批次号药品错误, %s', e);
        }
      },
      //小键盘回调
      callback(data){
        if(this.elItem) this.$set(this.elItem, 'measure_num', data.val);
      },
      //展示小键盘
      showKeyboard(item, event){
        this.elItem = item; //存储当前的item对象,回调时用来更改measure_num的值
        this.elInput = event.srcElement; //存储当前的HTMLElement对象,传递给小键盘组件
      ...
      inputKey(key){
        if(key==='0'){
          this.searchData.search_txt =  this.searchData.search_txt.length>0?this.searchData.search_txt.slice(0,this.searchData.search_txt.length-1):""
        }else {
          this.searchData.search_txt +=key;
        }
        this.selectAll(false);
      },
      async getList(){
      ...
      async beginMeasure(){
        ...
        if(this.measuring) {
          try{
            ...
            console.error('stop_measure_channel errpr, e=%o', e);  
          }
          return;
        }
        //开始盘点
        ...
        if(!slot_arr.length){
          console.error('no selected slot');
          return;
        }
        this.measuring = true;
        try {
          console.log('开始盘点药道: %o', slot_arr);
          await this.api.measure_channel(slot_arr);
        } catch (e) {
          console.error('盘点请求失败, err=%o', e);
        }
      },
      ...
      detail(item){
        ...
        if(item.bar_code)
          JsBarcode("#barcode")
            .options({font: "OCR-B"})
            .EAN13(item.bar_code.trim(), {fontSize: 14, textMargin: 0})
            .render();
