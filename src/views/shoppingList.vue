<template>
  <div class="shoppingList">
    <topbar :pageName="pageName" needConfirm=true></topbar>
    <div class="container">
      ...
      <div class="mask" v-show="failVisible">
        <div class="mask-dv">
          ...
          <div class="mask-box" style="top:6.46rem;">进行订单支</div>
  ...
  export default {
    ...
    methods:{
          ...
          if(!self.unload){
            self.successTimer = setInterval(()=>{
              if(self.successCountDown>1){
                ...
                self.$router.push({name:'waiting',params:{list:self.list,order_id:self.order_id,trade_no:self.trade_no}});
              }
            },1000);
          }
      ...
      async pickup(){
        console.log('开始取药');
        try{
          let list = this.cartData.cartProduct.map((item)=>{
            return{
              merchant_product_id:item.merchant_product_info.merchant_product_id,
              product_count:item.cartNum
            };
          });
          await this.api.pick_up_product( this.order_id,0 ,list);
          this.is_pickup = true;
          return true;
        }
        catch (e){
